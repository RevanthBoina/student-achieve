...
serve(async (req) => {
  ...
  try {
    ...
    // External AI moderation key removed. Use your own moderation provider if needed.
    ...
    if (userRecords && userRecords.length > 2) {
      const rejectedCount = userRecords.filter(
        (r) => r.status === "rejected",
      ).length;
    ...
    // NOTE: External Lovable AI moderation removed. If you want AI moderation,
...
function generateSuggestions(flags: string[]): string[] {
  ...
  if (flags.includes("SHORT_DESCRIPTION")) {
    suggestions.push(
      "Add more details about your achievement (minimum 100 characters recommended)",
    );
  }

  if (flags.includes("SHORT_TITLE")) {
    suggestions.push(
      "Provide a more descriptive title (minimum 10 characters)",
    );
  }

  if (
    flags.includes("MISSING_EVIDENCE") ||
    flags.includes("INVALID_DRIVE_LINK")
  ) {
    suggestions.push(
      "Upload video/photo proof to Google Drive and ensure the link is publicly accessible",
    );
  }

  if (flags.includes("DUPLICATE_SUBMISSION")) {
    suggestions.push(
      "This appears similar to a recent submission. Please ensure you are submitting a unique record",
    );
  }

  if (flags.includes("INAPPROPRIATE_CONTENT")) {
    suggestions.push(
      "Content may violate community guidelines. Please review and revise",
    );
  }

  if (flags.includes("LOW_QUALITY_CONTENT")) {
    suggestions.push(
      "Improve content quality with specific details, proper grammar, and clear descriptions",
    );
  }
