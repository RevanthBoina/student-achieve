...
export const createRecordSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Media files must be images (JPG, PNG, WEBP) or videos (MP4, WEBM)",
      path: ["mediaFiles"],
    },
  )
  .refine(
  ...
  .refine(
    (data) => {
      if (data.evidenceFiles && data.evidenceFiles.length > 0) {
        ...
        return data.evidenceFiles.every((file) =>
    ...
    {
      message:
        "Evidence files must be images (JPG, PNG, WEBP) or PDF documents",
      path: ["evidenceFiles"],
    },
  )
  .refine(
